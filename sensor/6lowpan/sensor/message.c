...
static uint16_t build_msg_header(uint8_t *buf, uint16_t len, msg_type_e msg_type,
                                 msg_method_e method)
{
...
uint16_t create_report_msg(uint8_t *buf, uint8_t *device_id, object_instance_t *obj)
{
    ...
    header_len = build_msg_header(buf, 0, TYPE_REQUEST, METHOD_REPORT);
    resource = object_instance_find_resource(obj, 5700);
    if (resource->resource_type->type == Integer) {
        parameters_len = sprintf(buf + header_len, "[[[\"%s\",%d]]]", 
                                 obj->name, resource->value.int_value);
    } else if (resource->resource_type->type == Float) {
        parameters_len = sprintf(buf + header_len, "[[[\"%s\",%f]]]", 
                                 obj->name, resource->value.float_value);
    } else if (resource->resource_type->type == String) {
        parameters_len = sprintf(buf + header_len, "[[[\"%s\",\"%s\"]]]", 
                                 obj->name, resource->value.string_value);
    } else {
        return 0;
    }
        
    ((msg_header_t *)buf)->len = parameters_len; 
...
uint16_t create_new_device_msg(uint8_t *buf, msg_type_e msg_type)
{
    ...
    data_len = build_msg_header(buf, 0, msg_type, METHOD_NEW_DEVICE);
    ptr = buf + data_len;
    ...
    ((msg_header_t *)buf)->len = (ptr - buf) - sizeof(msg_header_t); 
...
uint16_t create_get_config_msg(uint8_t *buf)
{
    ...
    data_len = build_msg_header(buf, 0, TYPE_REQUEST, METHOD_GET_CONFIG);
...
uint16_t create_log_msg(uint8_t *buf, uint8_t level, uint8_t *log)
{
    ...
    header_len = build_msg_header(buf, 0, TYPE_REQUEST, METHOD_LOG);
...
uint16_t create_subscribe_msg(uint8_t *buf, uint8_t *obj_name, uint16_t resource_id)
{
    ...
    header_len = build_msg_header(buf, 0, TYPE_REQUEST, METHOD_SUBSCRIBE);
